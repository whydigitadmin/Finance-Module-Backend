...
public interface ReconcileCashRepo extends JpaRepository<ReconcileCashVO, Long> {
	...
	@Query(nativeQuery = true, value = "select accountgroupname  from  groupledger where orgid=?1 and type='ACCOUNT' and active=1 group by accountgroupname")
	Set<Object[]> findByAccountName(Long orgId);
}
